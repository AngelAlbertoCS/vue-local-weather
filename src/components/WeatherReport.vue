    ...
    <div>
      ...
      >
        <div class="basis-1/4 text-left">
          <img :src="`https:${data.current.condition.icon}`" class="h-16 w-16" />
        </div>
        <div class="basis-3/4 text-left">
          ...
          <p>Wind: {{ data.current.wind_kph }} kph ({{ data.current.wind_degree }}°)</p>
        </div>
      </article>
      <div v-else class="p-4 text-white">Loading...</div>
    </div>
  </template>
